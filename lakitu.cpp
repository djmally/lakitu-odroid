...
//-----------------------------------------------------------------------------
{
    ...
    //pthread_t* pHandle = new pthread_t[devCnt];
    //pthread_attr_t* pAttr = new pthread_attr_t[devCnt];
    pthread_t *pHandle = new pthread_t();
    ...
    //ThreadParameter *threadParam = new ThreadParameter(devMgr[devIdx]);
    ThreadParameter *threadParam = new ThreadParameter(pDev);
    ...
    {
        pthread_attr_init( &pAttr[j] );
        // you can set the stack size like this: pthread_attr_setstacksize (&pAttr[j], 1024*1024);
        pthread_create( &pHandle[j], &pAttr[j], liveThread, ( void* )threadParams[j] );
    }*/
    pthread_attr_init(pAttr);
    pthread_create(pHandle, pAttr, liveThread, (void *)threadParam);
